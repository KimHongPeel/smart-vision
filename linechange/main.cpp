...
string src = "nvarguscamerasrc sensor-id=0 ! video/x-raw(memory:NVMM), width=(int)320, height=(int)160, format=(string)NV12, framerate=(fraction)40/1 ! nvvidconv flip-method=0 ! video/x-raw, width=(int)320,height=(int)160, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink";
